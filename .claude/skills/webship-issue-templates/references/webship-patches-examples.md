# Worked examples — webship/webship-patches issues & PRs

Distilled from the real history of [webship/webship-patches](https://github.com/webship/webship-patches) (issues/PRs up to #424, 2026). Use these conventions when filing new issues and PRs on webship-patches / drupal-core-patches. The [`webship-issue-templates`](../SKILL.md) templates still apply — this shows how they're worded in practice for patch work.
...
- **Target** — `<Module name> module` · `<Theme name> theme` (`The Gin Admin theme`) · `<machine> recipe` (`Drupal CMS Admin UI recipe`) · `<vendor/lib> library` (`openai-php/client library`, `e0ipso/twig-storybook library`) · `Drupal Core` · `Webship <x.y.x> profile`.
...
- **reason** (optional) — why now: `-- after <Module> <version> was released` (`#355 … -- After Trash 3.0.27 was released`), `- for Webship 11.0.x` (`#283`), `- for Drupal 10.6.2` (`#309`).

**Composer-plugin / infra PRs** use the Drupal commit-type form with the branch in brackets:

```
task: [10.1.x] require webship/drupal-core-patches: ~11 || ~12          (#388)
fix: [11.0.x] apply webship/drupal-core-patches by default (...)        (#394)
...
Issues use the default summary template. Real example ([#423](https://github.com/webship/webship-patches/issues/423)):

```markdown
### Problem/Motivation
Drupal core 11.4 removed `PathProcessorManager::addInbound()` … every request fatals:
`Error: Call to undefined method … addInbound()` — a global 500 on the Webship 9.2.x line.

#### Steps to reproduce
1. Build a Drupal 11.4 site that requires `webship/webship-patches:~9.2.0`.
...
Real example ([#424](https://github.com/webship/webship-patches/pull/424)):
...
A patch-file PR is terser — it just records the materialization ([#421](https://github.com/webship/webship-patches/pull/421)):

```markdown
Adds the materialized `.diff` of redirect MR !199 as a static, timestamped patch file on the
`patches` branch (via `composer web-ccup`), following `[package]--[date]--[issue]--[MR].patch`.
...
AI-Generated: Yes (Materialized via composer web-ccup; reviewed by <contributor>.)
```
...
**A published `.patch` is immutable.** The correct re-roll is [#421](https://github.com/webship/webship-patches/pull/421): a **new** MR (!199) → a **new** timestamped file → **supersede** the old one and update composer.json. Do NOT follow [#424](https://github.com/webship/webship-patches/pull/424)'s in-place reroll (same filename `…mr-109.patch`) — other projects pin that file and must keep resolving. Materialize every drupal.org MR through `ddev composer web-ccup` (or an equivalent producing a static, timestamped, standard-named file); never reference a raw MR URL. See the `webship-patches` / `drupal-core-patches` agents and the `patch-management` skill.
