...
This file is the vendor-neutral entry point for AI coding assistants working on `webship/webship-patches`. Read this first, regardless of which tool you are (Claude Code, Cursor, Codex, Aider, Continue.dev, Copilot Workspace, etc.). It captures the constraints that are NOT obvious from the code alone.

## What this repository is

A Composer plugin (`type: composer-plugin`, package name `webship/webship-patches`) layered on top of [`cweagans/composer-patches`](https://github.com/cweagans/composer-patches) (v1 or v2). It ships:

1. A curated `extra.patches` list (Drupal core + contrib patches vetted by Webship for Webship).
2. Three behaviors that upstream `cweagans/composer-patches` v2 dropped or never had:
   - **Allowlist** `allowed-dependency-patches` — default-deny, defaulting to `["webship/webship-patches"]`.
...
3. Two Composer commands (`web-ccup`, `web-ccupf`) that materialize remote MR URLs into local timestamped `.patch` files. These replace the old `drush webship:composer:cleanup:*` commands previously shipped in `webship_core`.
...
| `9.2.x`      | `~10.6.0`   | Webship `~9.2.0` (CKEditor 5)     |
| `9.1.x`      | `~10.6.0`   | Webship `~9.1.0` (CKEditor 4)     |
...
├── Command/                # web-ccup, web-ccupf Composer commands
├── Plugin/                 # WebshipPatchesPlugin entry point + activation
...
Install of webship/webship-patches failed.
```
...
If you remove v1 support, you break legacy Webship 9.x installs that pin v1.

### 3. Default-deny allowlist is a feature, not a bug.

`allowed-dependency-patches` defaults to `["webship/webship-patches"]`. This means a fresh project that adds `webship/webship-patches` and then later adds an unrelated contrib module with a stale `.patch` URL in its `extra.patches` will NOT abort the install — the unrelated contrib's patches are filtered out because it is not in the allowlist. Do not "fix" this by widening the default.
...
### 5. Patch filename convention is enforced by `web-ccup`.
...
- **A change ported to several branches keeps that one title plus a `- for Webship <x.y.x>` suffix per PR** — never a trailing `(<branch>)` tag, never an ad-hoc `ci: #<n> …` prefix. This holds for infrastructure changes (CI workflows, tests, docs) too.
...
- Repo: <https://github.com/webship/webship-patches>
- External docs: <https://github.com/webship/webship-patches/blob/11.0.x/docs/README.md>
