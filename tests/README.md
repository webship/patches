...
```bash
git fetch --depth=1 https://github.com/webship/patches.git patches:refs/remotes/upstream/patches
...
[`test.composer.json`](test.composer.json) is an ordinary Drupal project: it requires `drupal/core-recommended`, the modules this branch declares patches for (at the constraints Webship uses), and `webship/patches` itself from the checkout through a `path` repository. `composer install` therefore exercises the real plugin — allowlist, late activation, `cweagans/composer-patches` — against real packages.
