...
When `allowed-dependency-patches` is set to the default `["webship/patches"]`, `ignore-dependency-patches` is redundant — nothing else passes the allowlist.
...
Matching is done by URL string. The description (if you use the dict form) is informational only — `webship/patches` and the consumer can disagree on the description and the URL still matches.

### Ignoring Drupal Core Patches

`webship/drupal-patches` is an ordinary dependency that contributes patches through the dependency resolver, so the same `extra` keys control it — use `webship/drupal-patches` as the **source** package and `drupal/core` as the **target**:
...
The `patches-file` JSON has the same shape as the `patches` block at root level, just nested under a `patches` key.
