...
- **Recommended require:** `"webship/webship-patches": "~11.0.0"`
...
2. Wraps `cweagans/composer-patches` v2 to add three features missing from upstream v2:
   ...
   - **Allowlist** `allowed-dependency-patches` — only listed packages contribute dependency-declared patches. Default: `["webship/webship-patches"]`.
   - **`patches-ignore`** restored from cweagans v1 — drop a specific URL declared by a given dependency.
3. Provides Composer commands (`web-ccup`, `web-ccupf`) to convert remote merge-request URLs into timestamped local `.patch` files under `./patches/`.
...
- [`../.claude/agents/webship-patches.md`](../.claude/agents/webship-patches.md) — Claude sub-agent for installing, configuring, and troubleshooting `webship/webship-patches`.
...
- [`../.claude/skills/patch-management/SKILL.md`](../.claude/skills/patch-management/SKILL.md) — authoring, re-rolling, filename convention, and the Composer-native cleanup commands (`web-ccup` / `web-ccupf`).
...
| `9.2.x`      | `~10.6.0`   | Webship `~9.2.0`                  | <https://github.com/webship/webship-patches/blob/11.0.x/docs/README.md>         |
| `9.1.x`      | `~10.6.0`   | Webship `~9.1.0`                  | <https://github.com/webship/webship-patches/blob/11.0.x/docs/README.md>         |
| `no-patches` | n/a         | Plugin only, empty `extra.patches`| —                                                                          |
| `patches`    | n/a         | Patch files only, do not require  | —                                                                          |

To run with no Webship patches and manage your own list, require the `no-patches` branch (`webship/webship-patches: dev-no-patches`) — plugin still active, allowlist still enforced, but the curated list is empty.
